...
第二候補`face-reidentification-retail-0095`は公式FP32 IR（XML+BIN）の読込をOpenCV 5.0.0と4.13.0で実機試験した。両方ともAndroid AARにOpenVINO pluginがなく、`Backend (plugin) is not available: 'openvino'`で初期化できなかった。このためモデル本体をAPKへ含めず、モデル選択画面では実機非対応として表示する。
