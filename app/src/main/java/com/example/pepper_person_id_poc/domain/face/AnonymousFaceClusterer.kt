...
class AnonymousFaceClusterer(
    ...
    fun identify(trackId: String, embedding: FloatArray): AnonymousFaceResult {
        ...
        val mappedId = anonymousIdByTrackId[trackId]
        val mappedCluster = mappedId?.let { id -> clusters.firstOrNull { it.anonymousId == id } }
        val best = if (mappedCluster != null) {
            mappedCluster to cosineSimilarity(embedding, mappedCluster.centroid)
        } else {
            clusters
                .asSequence()
